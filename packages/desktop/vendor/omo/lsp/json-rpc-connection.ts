// Prime Desktop adaptation: bounded frames, pre-abort rejection and close disposal.
// Original source and MIT notices: ../NOTICE.md and ./LICENSE.
...
export class JsonRpcConnection {
	...
	private readonly handleData = (chunk: Buffer | string): void => {
		...
		if (this.inputBuffer.length + chunkBuffer.length > 8 * 1024 * 1024) { this.emitError(new Error("LSP frame limit exceeded")); this.dispose(); return; }
	...
	private drainInputBuffer(): void {
		while (true) {
			...
			if (contentLength === null || contentLength > 8 * 1024 * 1024) {
